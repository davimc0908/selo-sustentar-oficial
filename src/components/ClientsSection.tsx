  ...
  return <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        ...
        <Marquee pauseOnHover speed={40}>
          {clients.map((client, index) => <div key={index} className="relative h-full w-fit mx-[4rem] flex items-center justify-center">
              <div className="rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow duration-300 border border-border bg-slate-300">
                <img src={client.logo} alt={client.name} className="h-16 w-auto object-contain grayscale hover:grayscale-0 transition-all duration-300" />
              </div>
            </div>)}
        </Marquee>
